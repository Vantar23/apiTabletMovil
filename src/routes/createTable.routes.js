...
router.get('/crea-cadena', async (req, res) => {
    try {
        ...
        // Enviar la cadena a la URL especificada mediante POST con formato URL encoded
        const response = await axios.post('https://controlware.com.mx/recibe_avimex_tablet.asp', 
            qs.stringify({ cadena: cadena }),
            {
                headers: {
        ...
        if (response.status === 200) {
            res.status(200).json({ message: 'Cadena enviada con éxito', cadena, response: response.data });
        } else {
            res.status(response.status).json({ message: 'Error al enviar la cadena', response: response.data });
        }
